...
- (id)initWithCaptureID:(int)captureId {
    if (self == [super init]) {
        ...
        if ([_captureSession canAddOutput:captureOutput]) {
            ...
            NSLog(@"%s:%s:%d Could not add output to AVCaptureSession ",
                  __FILE__,
                  __FUNCTION__,
                  __LINE__);
        }
...
- (BOOL)startCaptureWithCapability:(const struct VideoCaptureCapability)capability {
    if (!_captureSession) {
        return NO;
    }
...
- (void)onVideoError:(id)e {
    // TODO(sjlee): make the specific error handling with this notification.
    NSLog(@"%s:%s:%d [AVCaptureSession startRunning] error.",
          __FILE__,
          __FUNCTION__,
          __LINE__);
}
...
- (BOOL)changeCaptureInputByUniqueId:(NSString*)uniqueId {
    ...
    if (!newCaptureInput) {
        ...
        NSLog(@"%s:%s:%d deviceInputWithDevice error:%s",
              __FILE__,
              __FUNCTION__,
              __LINE__,
              errorMessage);
...
didOutputSampleBuffer:(CMSampleBufferRef)sampleBuffer
    ...
    struct VideoCaptureCapability tempCaptureCapability;
    ...
    [self.delegate IncomingFrame:baseAddress length:frameSize capability:tempCaptureCapability time:0];
    
    CVPixelBufferUnlockBaseAddress(videoFrame, kFlags);
    
//    NSLog(@"capture frame size:%d", frameSize);
}
