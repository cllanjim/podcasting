...
bool OpenGles20::Render(struct AVFrame *frame) {
  if (texture_width_ != (GLsizei)frame->width ||
      texture_height_ != (GLsizei)frame->height) {
    SetupTextures(frame->width, frame->height);
  }
...
void OpenGles20::SetupTextures(GLsizei width, GLsizei height) {
  if (!texture_ids_[0]) {
